	...
	async fn is_public_network(&self) -> bool {
		...
		match result {
			...
			Err(e) => {
				...
				// For standalone/testnet, we default to false (testnet)
				// For production parachains, this should be true (mainnet)
				log::warn!(
					"Failed to fetch public network status from parachain: {e:?}. Assuming testnet (false)."
				);
				false
			},
		}
