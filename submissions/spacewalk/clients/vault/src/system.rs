...
impl VaultService {
	async fn new(
		...
		// Use the config file's network setting instead of the chain's setting
		// This ensures the wallet matches the Stellar overlay config
		let stellar_overlay_cfg = StellarOverlayConfig::try_from_path(&config.stellar_overlay_config_filepath)
			.map_err(Error::StellarRelayError)?;
		let is_public_network = stellar_overlay_cfg.is_public_network();
