...
const ITEMS: Item[] = [
  {
    id: "stellar-rocket",
    name: "Stellar Rocket",
    description: "3D printed rocket model. To the moon and beyond!",
    priceEth: 0.0003,
  },
  {
    id: "xlm-shield",
    name: "XLM Deflector Shield",
    description: "In-game item for the Soroban RPG. High defense stats.",
    priceEth: 0.0006,
  },
  {
    id: "soroban-book",
    name: "Soroban Spellbook",
    description: "Learn the ancient arts of smart contracts on Stellar.",
    priceEth: 0.0002,
  },
];
...
export default function Home() {
  ...
  // Price state for ETH/XLM conversion
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      try {
        ...
        setXlmPrice(data.stellar?.usd || 0.35);
      } catch (err) {
  ...
  // Convert ETH price to XLM (Lumens)
  const ethToXlm = (ethAmount: number): number | null => {
    if (!ethPrice || !xlmPrice) return null;
    const usdValue = ethAmount * ethPrice;
    return usdValue / xlmPrice;
  };
  ...
  async function handlePurchase(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (stellarAddress.trim().length < MIN_STELLAR_CHARS) {
      setFormError("Stellar address looks too short.");
      return;
    }
    ...
    try {
      ...
      await writeContractAsync({
        ...
        args: [stellarAddress.trim()], // Contract accepts generic "polkadotAddress" string param but we use it for Stellar addr here
        value,
      });
    } catch (error) {
    ...
    <main className="flex min-h-screen flex-col items-center bg-zinc-950 px-4 py-8 text-white">
      <div className="w-full max-w-5xl space-y-8">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-3xl border border-white/5 bg-zinc-900/50 p-6 backdrop-blur">
          <div>
            <p className="text-xs uppercase tracking-widest text-sky-400 font-bold mb-1">
              Sync Flow
            </p>
            <h1 className="text-2xl font-bold">Stellar Galactic Store</h1>
          </div>
        ...
        {!selectedItem ? (
          ...
          <section className="space-y-6">
            <div>
              <h2 className="text-xl font-semibold text-zinc-200">Stellar Artifacts</h2>
              <p className="text-zinc-500 text-sm">Purchase galactic items using ETH on Base.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {ITEMS.map((item) => (
                <MarketCard
                  ...
                  priceXlm={ethToXlm(item.priceEth)}
                  colorClass="sky"
            ...
            <div className="rounded-3xl border border-white/10 bg-zinc-900/70 p-8 shadow-2xl backdrop-blur">
              <header className="mb-8 flex gap-6 items-start">
                <div className="h-24 w-24 rounded-2xl bg-black/40 border border-white/10 flex items-center justify-center text-4xl shrink-0">
                  {selectedItem.name.includes("Rocket") ? "🚀" : selectedItem.name.includes("Shield") ? "🛡️" : "📖"}
                </div>
                <div>
                  <h2 className="text-2xl font-bold leading-tight">{selectedItem.name}</h2>
                  <p className="text-sky-400 font-mono text-lg mt-1">
                    {ethToXlm(selectedItem.priceEth)?.toFixed(2) ?? "..."} XLM
                    <span className="text-zinc-500 text-sm ml-2">({selectedItem.priceEth} ETH)</span>
                  </p>
                  <p className="text-zinc-400 text-sm mt-2">
                    Deposit ETH to complete purchase. You will receive <span className="text-white">XLM-Tokens</span> on the Stellar Testnet.
                  </p>
                </div>
              </header>
              ...
              <form className="space-y-6" onSubmit={handlePurchase}>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-zinc-300">
                    Shipping Address (Stellar)
                  </label>
                  <input
                    type="text"
                    value={stellarAddress}
                    onChange={(e) => setStellarAddress(e.target.value)}
                    className="w-full rounded-xl border border-white/10 bg-black/40 px-4 py-4 text-base text-white outline-none transition focus:border-sky-500 focus:ring-1 focus:ring-sky-500/50"
                    placeholder="GABC123...your Stellar address"
                  />
                  <p className="text-xs text-zinc-500">Your item receipt will be sent here.</p>
                </div>
                ...
                <div className="rounded-xl border border-white/5 bg-black/20 p-5 space-y-3">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-sky-400 font-medium">{estimatedNetEth.toFixed(4)} ETH</span>
                  </div>
                  <div className="pt-2 border-t border-white/5 flex justify-between items-center">
                    <span className="text-zinc-400 text-sm">Receipt</span>
                    <span className="text-sm font-bold text-white">
                      {estimatedAssetPayout} <span className="text-sky-400">XLM Units</span>
                    </span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full rounded-2xl bg-sky-500 px-4 py-4 text-lg font-bold text-black transition hover:bg-sky-400 disabled:cursor-not-allowed disabled:opacity-50 shadow-lg shadow-sky-900/20"
                >
                  {isWriting ? "Confirming Order…" : `Pay ${selectedItem.priceEth} ETH`}
                </button>
              ...
              {(hash || isConfirmed) && (
                <div className="mt-6 rounded-xl border border-sky-500/20 bg-sky-500/5 p-4 text-center">
                  <p className="text-sky-200 font-medium mb-2">
                    {isConfirmed ? "Purchase Successful! 🚀" : "Order Submitted"}
                  </p>
                  <p className="text-xs text-sky-400/70 mb-3">Check contract interactions for confirmation.</p>
                  {hash && (
                    <a
      ...
      <p className="mt-6 text-center text-xs text-zinc-500">
        Need test liquidity? Wire small amounts first, verify XLM arrives, then
        scale.
      </p>
    </main>
  );
